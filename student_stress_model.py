...
student_type = st.selectbox("Student Type", ["School", "College", "Working-Student"])
...
attendence = st.number_input("Attendence (%)", min_value=0.0, max_value=100.0, value=80.0)
exam_pressure = st.slider("Exam Pressure", 1, 10, 5)
family_type = st.slider("Family Support", 1, 10, 5)

student_school_map = {"School": 0, "College": 1, "Working-Student": 2}
student_type = student_school_map[student_type]
...
if st.button("Predict"):
    new_student = pd.DataFrame({
        ...
        "Family_Support": [family_type]
    })
    new_student_scaled = scaler.transform(new_student)
    prediction = model.predict(new_student_scaled)
    if prediction[0] == 1:
        st.success("Predicted : High Stress")
    else:
        st.success("Predicted : Low Stress")